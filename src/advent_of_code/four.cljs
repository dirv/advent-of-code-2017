...
(defn- simple-split [phrase]
  (filter #(not= % "") (clojure.string/split phrase #" ")))

(defn- sort-split [phrase]
  (map (comp seq sort (partial apply str)) (simple-split phrase)))

(defn valid? [split-func phrase]
  (let [words (split-func phrase)
        freqs (distinct (vals (frequencies words)))]
    (and (= 1 (count freqs)) (= 1 (first freqs)))))

(defn run [split-func]
  (count (filter true? (map (partial valid? split-func) (clojure.string/split-lines passphrases)))))
