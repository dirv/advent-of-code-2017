...
(def input "86  440 233 83  393 420 228 491 159 13  110 135 97  238 92  396
...
(def rows (clojure.string/split-lines input))

(defn values [row]
  (map int (filter #(not= % "") (clojure.string/split row #" "))))

(defn max-difference [xs]
  (- (apply max xs) (apply min xs)))

(defn- divisible-by [xs d]
  (some #(when (and (not= d %) (zero? (mod d %))) (/ d %)) xs))

(defn only-division [xs]
  (some (partial divisible-by xs) xs))

(defn checksum [row-checksum]
  (apply + (map (comp row-checksum values) rows)))

(defn part-one []
  (checksum max-difference))

(defn part-two []
  (checksum only-division))
